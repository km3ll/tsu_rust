...
fn basic_error_handling() {
	...
	- We can't react programmatically to different error cases, as distinguishing between user errors vs internal errors
	- Users should get a friendly error message if their input is invalid, because that's something they can fix
 	...
 	- As Devs, not only do we want to see details of errors, but also we want to see an error chain, meaning the top-level error plus all the errors that caused it
	---"#;
    println!("{n1}");
}
