...
fn deref_coercion_base() {
    let n1 = r#"
    pod: DeRef Coercion (DeReference)
    - The type we're passing in is different from the type accepted
    - To coerce a reference of one type into a reference of another type `&Box -> &String -> &str`
    - Only works on types that implement the `Deref` and `DerefMut` traits
    - The compiler generates this sequence of deref calls implicitly
    - `Deref` and `DerefMut` should only be implemented on smart pointer types
    - DeRef operator is (`*`)
    ---"#;
    println!("{n1}");
