...
async fn get_all_books(
    ...
    if let Ok(books) = crate::db::all_books(&cnn).await {
        Ok(Json(books))
    } else {
        Err(StatusCode)
    }
...
async fn get_book(
    Extension(cnn): Extension<SqlitePool>,
    Path(id): Path<i32>
) -> Result<Json<Book>, Stat> {
    if let Ok(book) = crate::db::book_by_id(&cnn, id).await {
        Ok(Json(book))
    } else {
        Err(StatusCode::I)
    }
